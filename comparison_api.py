...
def generate_context_text(record, input_value):
    ...
    if ratio < 0.5:
        ...
        return f"Your recipy are about {percent}% of those from {record['name']}."
    elif ratio < 0.9:
        # Input is somewhat lower than the stored value.
        return f"Your recipy are {record['name']} (around {percent}%)."
    elif ratio <= 1.1:
        # Input is roughly equivalent.
        return f"Your recipy are roughly equivalent to {record['name']}."
    elif ratio <= 2:
        # Input is higher but within a moderate range.
        excess = round((ratio - 1) * 100)
        return f"Your recipy are approximately {percent}% of those from {record['name']}, which is about {excess}% more."
    else:
        # Input is much higher.
        return f"Your recipy are significantly higher than those from {record['name']} (over {round(ratio, 1)} times as much)."
